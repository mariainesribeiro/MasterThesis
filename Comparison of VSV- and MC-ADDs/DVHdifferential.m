...
%
%   Returns:
%       Y_volume - [1 x max_D+1 double] volume axis of the differential DVH
%       max_D    - [integer] integer maximum D within the VOI in the ADDs
%
%  ---------------------------- EXAMPLE -------------------------------------
%  ADD = load_untouch_nii('PatientXXX_ADD.nii');
%  VOI = load_untouch_nii('PatientXXX_VOI.nii');
%  [Y_volume, max_D] = DVHdifferential(ADD,VOI);
%  X_dose = 0:max_D;
...
Y_volume = 0:max_D;
for d = 0:max_D
    Y_volume(d+1) = sum(VOI_ADD_array >= d & VOI_ADD_array < d+1 )/VOI_vox;
end
end
